...
title: Донастройка (+ доп. настройки)
id: oonfigure
...
Вся такая донастройка происходит внутри метода
<div align="left">
```kts
<npcID>.configure {
  ... // Т.е. здесь!
...
Пдерживаются все параметры, что были [При создании НИПа](./0-create.mdx), но кроме [`name`](./1-configure.mdx#дополнительные-возможности). Для такого предусмотрен другой метод.
