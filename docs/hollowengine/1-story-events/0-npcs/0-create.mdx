...
Если вам нужна документация для Legacy, вам нужно сюда: [Hollow Engine Legacy Docs](/docs/Legacy/hollowengine/story-events/npcs/create)
:::
...
![vitalik-start](@site/static/img/doc/titles/CreateNPC0.png)
...
В `[] (квадратных скобках)` ты указываешь название текстуры, которая есть на модели.
У модели может быть несколко текстур. Чтобы узнать какие текстуры есть в модели - используйте команду: `/hollowengine model <путь до модели>`.
['/Команды'](../../0-begin/2-commands.mdx):

![textureName](@site/static/img/doc/menus/layersName.png)
...
<div style={{"display":"flex"}}>
<div style={{"align-items":"flex-start","margin":"20px"}}>
