...
Всё ниже напианное пижется в методе `init()`:
<div align="left">
```kts
fun init() {
...
ITEMS.register(newItemId: String) {
  Item(Item.Properties()
    .stacksTo(8) // Теперь в одну ячейку помещается 8 предметов
    .tab(HOLLOWENGINE_TAB) // Предме тбудет ледать в табе мода HollowEngine
  )
}
```
---
<div align="center">
<ToggleText buttonText="Дополнительные параметры для предмета">
<div align="left">
...
<ToggleText buttonText="Дополнительные функции для предмета">
...
именем, который вы укзалаи при создании регистратора.
</div>
:::note Пример
<div align="left">
