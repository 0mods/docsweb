...
/*Запись в переменную */
val myValue = <players>.input().message // Запишет в переменную сообщение
val myValue = <players>.input().player // Запишет в переменную игрока, который отправил сообщение
```
...
val meMessage = players.input().message
val mePlayer = players.input().player
```
