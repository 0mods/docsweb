...
:::note НИП будет двигаться до ближайшего игрока из команды - бесконечно | Щаблон
```jsx
...
:::note НИП скажет эти слова | Шалон
```jsx
<npcID> say { "<text>" }
```
:::
...
> Если у НИПа не отображается предметы в руках, то вот помощь -> [Отображение предмета в руках НИПа](../Tools/ItemHandVisible)
:::
